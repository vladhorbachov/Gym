package com.gymshark.ui.home.pulse.heartrate
...
import com.gymshark.ui.home.pulse.PulseFragment
            ...
            override fun onTick(millisUntilFinished: Long) {
                ...
                val thread = Thread {
                    ...
                    if (detectValley()) {
                        ...
                        sendMessage(PulseFragment.MESSAGE_UPDATE_REALTIME, currentValue)
                    }
            ...
            override fun onFinish() {
                ...
                if (valleys.size == 0) {
                    mainHandler.sendMessage(
                        Message.obtain(
                            mainHandler,
                            PulseFragment.MESSAGE_CAMERA_NOT_AVAILABLE,
                            "No valleys detected - there may be an issue when accessing the camera."
                        )
                    )
                    return
                }
                val pulse = 60f * (detectedValleys - 1) / Math.max(
                    1f,
                    (valleys[valleys.size - 1] - valleys[0]) / 1000f
                )
                val currentValue = String.format(
                ...
                sendMessage(PulseFragment.MESSAGE_UPDATE_FINAL, pulse)
                cameraService.stop()
            }
        }
//        activity.setViewState(PulseFragment.VIEW_STATE.MEASUREMENT)
